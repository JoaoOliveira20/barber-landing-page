import { demoBarContent, demoWhatsappUrl } from "@/config/site";
...
export function DemoBar() {
  return (
    <div className="fixed inset-x-0 top-0 z-50 flex h-9 items-center justify-center gap-2 overflow-hidden bg-surface-sunken px-3 text-center text-xs text-muted sm:px-4">
      <span className="truncate">{demoBarContent.label}</span>
      <span aria-hidden="true" className="hidden sm:inline">
        ·
      </span>
      <span className="hidden sm:inline">{demoBarContent.author}</span>
